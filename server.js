...
app.post('/api/upload-students', upload.single('file'), async (req, res) => {
  try {
    ...
    const yearIdx = getColIndex(['year', 'sec', 'class']);
    ...
    for (let i = headerRowIdx + 1; i < rows.length; i++) {
      ...
      const yearSec = yearIdx !== -1 && row[yearIdx] ? String(row[yearIdx]).trim() : 'III / A';
...
app.post('/api/approve-pass', async (req, res) => {
  try {
    ...
    const newPass = new Pass({
      ...
      status: 'Approved',
      approvalTime: now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
      expiresAt: expiresAt
    });
...
app.post('/api/scan-pass', async (req, res) => {
  try {
    ...
    const exitFormattedTime = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    pass.status = 'Exited';
